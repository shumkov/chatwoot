...
This repo is **UMI's fork of Chatwoot** — a small stack of patches on top of an upstream stable tag (currently `v4.16.0`). Before any non-trivial change, read **`CONTRIBUTING-UMI.md`** (the patch recipe + golden rules), **`UMI-PATCHES.md`** (the patch registry), and **`FORK.md`** (branch/build/deploy model). The rules below are load-bearing — they keep the fork rebasable onto new upstream releases:
