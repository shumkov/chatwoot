...
| 14 | Historical FB/IG archive import | `umi/app/services/fbig/history_import_*.rb`, `umi/app/services/fbig/{contact_inbox_platform_evidence,history_approval_manifest,history_profile_backfill_service,history_state_*,profile_*,avatar_*,contentless_fingerprint,unavailable_message_thread_fingerprint,recovered_thread_targets,production_first_*,sanitized_koala_api,typed_value_digest}.rb`, `umi/app/services/fbig/message_heal_service.rb`, `lib/tasks/umi_fbig_history_import.rake`, `script/umi_fbig/{build_programs.rb,programs/*.sh,support/*}`, `db/migrate/*_add_unique_contact_avatar_attachment_index.rb`, `db/schema.rb`, focused specs in `spec/services/umi/fbig/`, `spec/script/umi_fbig/`, `spec/models/active_storage/`, and `spec/lib/tasks/rake/`; docs: `docs/UMI-FBIG-{HISTORY-IMPORT-SPEC,FULL-HISTORY-EXPANSION-SPEC,FULL-HISTORY-RUNBOOK}.md` | One-time, digest-bound import of every Messenger/Instagram thread and message Meta still exposes. History and profile enrichment are separate: approved history runs are always profile-deferred, append only missing messages, create no empty archives, durably reconcile interrupted attachment uploads, and accept unavailable bodies only by exact aggregate fingerprints; unavailable message-thread acceptance is now literal zero. The Graph client gives only the initial message edge a shared bounded transient retry budget, while authentication failures and later-page failures remain fail-fast. Exact digest-only recovered-thread targets prove that the two formerly failed Instagram threads are listed and message-cursor exhausted without exposing their raw ids. A later rate-aware profile task scans stable participants without rereading messages, repairs only exact generated placeholders, fills only blank/absent profile fields, preserves avatars, and seals PII-free pre/post/avatar-staging evidence. Snapshot-bound Instagram placeholder seeds use bounded parsing, canonical platform evidence revalidated before lookup and under write locks, attempt-bound audit artifacts, and per-result counter conservation rather than a fixed cardinality. Production normally requires terminal clone acceptance and checksum-bound dry pairs; the operator-authorized one-time production-first path instead uses a distinct release/backup/R2-provenance manifest, deterministic request/binding generators, self-contained immutable sidecars, a lock-held unique global result head, backup-bound per-platform state snapshots, complete authorization/approval/result lineage, strict contentless-only revisions, resumable attributed attempts, byte-associated platform evidence (with one exact legacy-program exception), and no fabricated clone evidence. Both modes retain exact one-off/running image-and-commit proof, crash-adoptable single-platform graph deltas, cutoff-to-completion checkpoints, delivery audits between profile windows, separate terminal zero-write history passes, a terminal zero-write profile pass, and final repeatable-read live reconciliation with audited platform totals and exact remaining-placeholder classification. Strict immutable approvals, same-process DB identity, a renewable writer lock, callback-free inert archives, coordinated DB/storage backups, and a partial unique Contact/avatar index make apply and recovery fail closed. | Upstream provides a callback-safe Meta history/profile importer with immutable acceptance evidence and equivalent recovery controls; or UMI completes and audits the one-time migration, then removes the manual importer while retaining any still-shared lock/index behavior. |
...
An exact Instagram-only initial message-connection error
(`HTTP 400`, code `-1`, subcode `2207085`, `OAuthException`) is a classified
release-bound omission only when its full per-thread fingerprint matches the
approval. It creates no contact, archive, or message. Messenger, later-page,
or changed error shapes remain fatal. Every successful scan proves
`listed_threads = message_cursor_exhausted_threads +
classified_omitted_threads + failed_threads`, with zero failed, partially
paginated, or uncategorized threads.
