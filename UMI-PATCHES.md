...
| 20 | Meta ad attribution capture | `umi/app/builders/fbig_ad_attribution.rb`, `config/initializers/zz_umi_fbig_ad_attribution.rb`, `db/migrate/20260811000000_create_umi_meta_ad_attribute_definitions.rb`, `umi/app/controllers/webhooks/shopify_compliance.rb`, `spec/builders/umi/fbig_ad_attribution_spec.rb`; docs: `docs/UMI-FBIG-AD-ATTRIBUTION-SPEC-R4.md` | Chatwoot captures Meta's ad `referral` object for WhatsApp (`whatsapp/incoming_message_service_helpers.rb:74`) and Twilio (`twilio/referral_params_helper.rb`) but **not** for Facebook/Instagram, so a conversation started by clicking an ad is indistinguishable from an organic one. Prepends store the whole object on `message.content_attributes[:referral]` and promote `ad_id`/`ref`/`ads_context_data.ad_title` to conversation custom attributes, with a migration seeding the three `CustomAttributeDefinition` rows (an undefined key renders nothing in the sidebar and makes an automation rule unsavable). **Meta nests `referral` inside `message`, not beside it** — verified against a production payload; reading the top-level key returns `nil`, indistinguishable from organic, and specs pin the nesting (4 fail if it regresses). Promotion runs *after* the builder transaction commits, because a rescued DB error inside it would abort the transaction and kill the customer's message at COMMIT. Only `source == 'ADS'` promotes — Meta reuses the same key for Instagram Shops product taps. Erasure counterpart `purge_for` strips both layers. Measured: Instagram delivers referrals **without** `messaging_referrals` subscribed (live field set is 6 fields, that one absent); Facebook unverified. | Upstream captures Meta `referral` for the FB/IG channels as it already does for WhatsApp and Twilio. |
