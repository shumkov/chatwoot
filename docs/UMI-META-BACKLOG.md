...
| | **Captured conditions (2026-08-10).** `Lead reminder`: lead → *Qualified*, then **12h** inactive → internal admin note (invisible to customer) + mark unread, optional follow-up flag. `Leads – Lost leads`: lead in *Intake* with **7 days** no messages → set stage **Lost**. `Leads – Booked leads`: booking accepted or order marked "Booked" → set stage **Qualified/Converted** + matching label. **CAPTURED 2026-08-12** (same screen re-confirms `Questions and responses` **OFF**, all five lead automations **ON**): `Leads – Ordered leads` — trigger *"creates an order, updates the status of an order **or receives messages about placing an order**"*, action stage **Converted** + label; so it is **not purely order-driven**, yet still produced Converted 0 even though 12 live conversations carry an inbound สั่ง/สั่งซื้อ — both branches measured at zero. `Leads – Responsive leads` — trigger *"**exchanges 5 messages** with a lead in the Intake stage"*, action stage **Qualified** + label; this is what produces Qualified 12/30 = 40 %, and an independent count puts the same threshold at **39 % of live FB/IG conversations (42/108)**. | | |
| | **BLOCKER for D6 — Chatwoot has no time-based automation trigger.** `AutomationRuleListener` supports only `conversation_created`, `conversation_updated`, `conversation_opened`, `conversation_resolved`, `message_created`. Two of the five Meta automations are purely time-based (7-day inactivity → Lost; 12h inactivity → reminder) and **cannot be expressed with stock rules**. Options: Chatwoot **SLA policies** (time-based breach handling already exists, see `lib/tasks/apply_sla.rake`), or a UMI scheduled job in the same shape as the profile refresher. **ANSWERED 2026-08-12: neither.** Stock `Account#auto_resolve_after` + `auto_resolve_label` already sweeps on inactivity every 5 minutes and applies a label (`app/jobs/conversations/resolution_job.rb:5-13`); SLA is unlicensed here (`community` plan, `feature_enabled?('sla') == false`), cannot express "N days silent" (its only knobs are frt/nrt/rt), and performs no actions at all. Spec: `docs/UMI-META-AUTOMATION-PORT-SPEC.md`. | code | **D6 is configuration, not a patch** — 1 of 5 automations ported, 4 dropped, zero new code. The one code dependency is deploying patch 20, which is **not running in production** (migration `20260811000000` unapplied, 0 conversation custom attribute definitions). |
| | **The timers are NOT the valuable half — r2, 2026-08-12.** The five automations inventoried above are outranked by the **sixth** row, `Questions and responses`, recorded here as OFF. Measured on the live tap cohort: agents answer ad taps in a **1.1 min** median (faster than organic's 17.9 min) and 20 of 21 get a reply, yet customer follow-up is **4/20 = 20 %** against **44/55 = 80 %** for non-ad conversations (Fisher p ≈ 3.3e-6); 14 of 21 tap threads contain exactly one inbound message ever. Cause is answer relevance, not speed: the three tap literals are stable (`1. สนใจรับส่วนลด 10%…`, `2. ขอแนะนำสินค้าสำหรับวันแม่`, `3. ขอแนะนำสินค้าขายดีของ UMI`, 29 of 31 taps), but only **two** reply blocks exist — **no Mother's-Day block at all**, so that tap gets best-sellers or the coupon; and the 10 % tap has produced **0 follow-ups in 9 replies** (`WELCOME10` overall: 6 replies across 21 conversations). Chatwoot can express question-matched replies as `message_created` + `content contains` → `send_message`, and crucially an automation reply does **not** set `first_reply_created_at` (`human_response?` excludes `automation_rule_id`), so the thread stays in Unattended. **Also: the D1 greeting now asks "which product model are you interested in?" — the exact bounce-back this is about — and fires before the auto-answer.** | code + ops | **Reversed ranking: question-matched replies first, timers second and gated.** Turning on 7-day auto-resolve today would close all 21 tap conversations within a week — the population the fix must be measured on. |
...
| ~~**D11**~~ | ~~**Agent-side contact linking is broken.**~~ **NOT A DEFECT — closed 2026-08-11.** The 422 is real and the mechanism was described correctly (`ContactsController#update` does a bare `assign_attributes` + `save!`; `Contact` validates `email`/`phone_number`/`identifier` uniqueness scoped to the account, `contact.rb:50-56`), but the conclusion was wrong. | — | **My earlier claim that this "blocks *every* manual identity-resolution path" was false.** Three things check out: the exception **is** rescued (`request_exception_handler.rb:11`) and renders a 422 carrying `"Email has already been taken"` — an intelligible message, not a crash; **contact merge is fully wired end to end** — a `MERGE` tab in `ContactManageView.vue:45,180` against `resource :contact_merge` (`config/routes.rb:55`) backed by `ContactMergeAction`; and merge is the *correct* operation for linking a Meta contact to a Shopify one, since typing a duplicate email should fail. So the path is clunky, not blocked, and nothing here gates the capture work. Only residual gap: the error names neither the conflicting contact nor merge as the remedy. Small UX polish, not a blocker — build only if agents actually hit it. |
| **D12** | ~~**Free deterministic win: draft-order invoice tokens.**~~ **REFUTED at scale 2026-08-11, and BLOCKED.** The verification was n=1 and did not generalise: of **77 orders, exactly 1** carries an `/invoices/<token>` in `landing_site` (that same #1583), and it matches **none** of the 14 tokens agents actually pasted into DMs. Join yield: **0 orders, 0% of revenue.** | blocked | The correct join key is the **DraftOrder's own `invoice_url` + `order_id`**, not the order's `landing_site` — but `GET /draft_orders` returns *"This action requires merchant approval for `read_draft_orders` scope."* **Needs a Shopify scope grant before it can even be measured.** Also note the 14 chat tokens matched no completed order at all, so a chunk of invoices sent are simply never paid — the ceiling here may be well under the 9.6% first assumed. |
| **D16** | **Web orders already carry Meta attribution — nobody is reading it.** 26 of 77 orders have `note_attributes`, and they include real UTM data: `utm_source: "ig"`, `utm_id: "120252078374130415"` (a Meta campaign id). All 29 `web` orders also have full identity (29/29 email, 28/29 customer). | code | **Cheapest remaining win.** No capture work, no new scopes — the data is already on the order. Covers the 44.6% of revenue that goes through web checkout. Read `note_attributes` on order ingest and join to the ad. |
| **D17** | **The largest order bucket is anonymous and unexplained.** 37 of 77 orders (~44% of revenue) come from `source_name: "325329387521"` — an app id nobody has identified — and they carry almost no identity: **email 4/37, phone 4/37, customer 8/37**, no `note_attributes`, no `landing_site`. | product | This is the real in-DM sales population and it is the hard half of D8. No automated join can reach it because there is nothing to join *on*. Two questions: what app is `325329387521`, and can whoever creates these orders be asked to stamp a conversation reference (`note_attributes`, `note`, or a tag) at creation time? That is a workflow change, not a code change. |
